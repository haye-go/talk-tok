...
const DEFAULT_PROMPTS = [
  ...
  },
  {
    ...
    userTemplate: [
      ...
      "Fight mode: {{mode}}",
      "Status: {{status}}",
      "Post that initiated this fight: {{sourcePostJson}}",
      "Turns: {{turnsJson}}",
      "",
      "Evaluate whether the fight addressed the original post's actual claim.",
      "Assess whether the challenger attacked the strongest version of that claim, and whether the defender clarified, protected, or improved it.",
      "Call out what evidence or reasoning would make the rebuttal stronger.",
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "Focus on participation, Q&A-aware reasoning, contribution trace, and development over time.",
      "This session may be Q&A-oriented. If the participant mostly posted questions, evaluate those contributions as questions rather than argumentative essays.",
      "Good questions may clarify scope, expose assumptions, request examples, challenge ambiguity, or open a useful answer path.",
      "Do not criticize a question for lacking evidence, supporting context, or a full argument unless it was framed as a claim.",
      "Use concrete references to what the participant actually asked or contributed.",
      "Avoid harsh grading language such as lacked reasoning, failed to engage, or did not demonstrate deeper engagement.",
      "Use telemetry only as soft context, not as proof of misconduct.",
      "Keep the tone constructive, specific, student-facing, and non-accusatory.",
      "Return strict JSON matching the requested schema.",
    ].join("\n"),
    userTemplate: [
      "Session title: {{sessionTitle}}",
      "Opening prompt: {{openingPrompt}}",
      "Contribution mode: {{contributionModeJson}}",
      ...
      "For reasoningBand, assess the quality of thinking shown through questions and comments. For questions, reasoning means clarity, relevance, specificity, and whether the question creates a useful answer path.",
      "The summary should be 2-3 constructive sentences. If the participant mostly asked questions, frame it around the usefulness and clarity of those questions.",
      "The contributionTrace must be concrete. Name the main topics or questions the participant contributed. Do not use generic text like 'included in the discussion record'.",
      "The argumentEvolution field should describe development over time. For mostly Q&A participation, describe how the questions shifted, repeated, or stayed focused, using neutral wording.",
    ].join("\n"),
    modelOverride: "openai:gpt-4.1",
    variablesJson: {
      temperature: 0.25,
