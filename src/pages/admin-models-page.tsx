    ...
    <AdminShell title="Providers & Models" description="Manage LLM providers, model assignments, and pricing metadata.">
      <div className="grid gap-4">
        ...
        {models && models.length === 0 && (
          <Card>
            <p className="text-sm text-[var(--c-muted)]">No model settings configured. Run seed defaults to create initial entries.</p>
          </Card>
        )}
        ...
        {models && models.length > 0 && (
          <>
            <Card
              ...
              action={<Badge tone="neutral"><GearSix size={10} className="mr-0.5 inline" /> {models.length} models</Badge>}
            >
              <div className="divide-y divide-[var(--c-hairline)]">
                {models.map((model) => (
                  <div key={model._id} className="flex items-center justify-between py-2.5 text-sm">
                    <div>
                      <p className="font-display text-sm font-medium text-[var(--c-ink)]">{model.model}</p>
                  ...
                  <tbody className="text-[var(--c-body)]">
                    {models.map((model) => (
                      <tr key={model._id} className="border-b border-[var(--c-hairline)]">
                        ...
                        <td className="py-2 pr-4">${(model.cachedInputCostPerMillion ?? 0).toFixed(2)}</td>
